          ...
          <div className="lg:col-span-7 space-y-4">
            {faqs.map((faq, index) => {
              ...
              return (
                <div
                  key={index}
                  className="bg-white border border-slate-100 hover:border-primary/10 rounded-2xl overflow-hidden transition-all duration-300 shadow-xs"
                >
                  <button
                    onClick={() => setOpenIndex(isOpen ? null : index)}
                    className="w-full flex items-center justify-between p-5 text-left font-bold text-slate-805 text-base sm:text-lg cursor-pointer select-none outline-none focus:text-primary transition-colors"
                  >
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      ...
                      >
                        <div className="px-5 pb-5 pt-1 text-xs sm:text-sm font-semibold text-slate-400 border-t border-slate-50 leading-relaxed">
                          {faq.answer}
                        </div>
                      </motion.div>
                    )}
